...
int should_we_posix() {
#ifdef _WIN32
	wchar_t* buffer = new wchar_t[65535];
	size_t bufsize = 65535;
	errno_t retcode = _wdupenv_s(&buffer, &bufsize, L"POSIXLY_CORRECT");
	if (retcode != 0) { return 0; } // If it's not there, assume no.
	else { return !!buffer; }
#else
	return !!_wgetenv(L"POSIXLY_CORRECT");
#endif
}
...
static const char *_getopt_initialize_a (const char *optstring, struct _getopt_data_a *d, int posixly_correct)
{
	...
	d->__nextchar = NULL;
	d->__posixly_correct = posixly_correct | should_we_posix();
	if (optstring[0] == '-')
	{
